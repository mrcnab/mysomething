<?
	...
	if(isset($_REQUEST['searchSubmit']) && $_REQUEST['flag'] == 'search'){
		...
		$q = " SELECT * FROM `title_dev_customers` WHERE userType = 'Classified' AND `firstName` LIKE '".$searchResult."' OR `email` LIKE '".$searchResult."'  order by  ".$sorting;
		$q1 = "SELECT count( * ) as total FROM `title_dev_customers` WHERE userType = 'Classified' AND `firstName` LIKE '".$searchResult."' OR `email` LIKE '".$searchResult."'";
...
?>
<h1>Classified Users Management</h1>
