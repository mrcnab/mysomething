<?php 
	...
	if($_POST && $customerId > 0 ){
			...
			if($package_status == 1){
				...
				$getOrderStatus	=	$advert_obj->getOrderStatusByCustomerId($customerId);

				$checkCustomerIdFromOrderTable	=	$getOrderStatus['customerId'];
			
			if($checkCustomerIdFromOrderTable	==	$customerId){

				$displayMessage = "<div class='bad_msg' style='margin-top:10px;'>You have already one advert with us, you only can have one advert at one time.</div>";
		...
		if($getCustomerInfo['userType'] != 'Directory'){   
			...
			$AdType	=	'Classified';
			$cust_email_top_line = 'Thank you for uploading your classified on My Something Old Something New. <br /> We hope that you find a buyer soon! Your classified will be activate within 24 hours';
			$cust_email_btm_line = 'Remember, your classified will appear on the site for 30 days.  After that time you will need to re-list your classified. ';
	  		...
	  		$AdType	=	'Directory';
			$cust_email_top_line = 'Thank you for uploading your advert on the My Something Old Something New directory.  Your advert will be activate within 24 hours ';
		...
		$MessageBody = "<table width='755' border='0' align='center' cellpadding='0' cellspacing='0' style='border:0px solid #41ad49;'>
				  <tr>
					...
					<td ><img src='http://mysosn.co.uk/images/logo.png'/></td>
...
The MySOSN team
			...
			else
			{
				...
				$content_obj->add_order_details( $customerId, $packageId, $price);
