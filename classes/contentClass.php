...
function getTopBanners( )
	...
	function add_order_details( $customerId, $package_id, $price)
	{
		$q = "INSERT INTO title_dev_order ( `customerId` , `package_id`,  `price` ,  `status` , `addeddate`)		
		 VALUES( '".$customerId."', '".$package_id."', '".$price."', 'Un Paid','".date('Y-m-d H:i:s')."')";		
		$r = $this -> db -> insertRecord( $q );
			if( $r != false )
				return true;
			else
	...
	}	//	End of function add_user_record( $fullname, $email, $phone, $date, $persons, $hours, $minutes, $instructions, $status )
	

	function updateUserStatusOnPayment( $customerId )
	{
		$q = "UPDATE title_dev_customers SET  `status` = 1, `modifiedDate` = '".date('Y-m-d H:i:s')."' WHERE customerId = ".$customerId;
		$r = $this -> db -> updateRecord( $q );
		if( $r != false )
			return true;
		else
	...
	function updateCustomerTotalCount($customerId){
	
		$q = "UPDATE title_dev_customers SET `advertCount` =advertCount +1 WHERE customerId = ".$customerId;
		$r = $this -> db -> updateRecord( $q );
		if( $r )
			return true;
		else
