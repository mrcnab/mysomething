...
class adverts
{
	...
	function getPackageDetailAdvert($userType)
	{
		$q = "SELECT * FROM title_dev_packages WHERE  `package_type` = '".$userType."' ";
		$r = $this -> db -> getSingleRecord( $q );
		if( $r )
			return $r;
		else
			return false;
	}	//	End of function get_advertisment_info( $package_id )
	...
	function getOrderStatusByCustomerId( $customerId)
	{
		$q = "SELECT * FROM title_dev_order WHERE customerId = ".$customerId;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r )
			return $r;
		else
	...
	function getPackageDetailByPackageId($packageId)
	{
		$q = "SELECT * FROM title_dev_packages WHERE  package_id = ".$packageId;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r )
			return $r;
		else
	...
	{
		$q = "SELECT * FROM title_dev_order WHERE order_id  = ".$orderId;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r )
			return $r;
		else
...
function display_saved_Customer_Advert_listing( $title_dev_customer_saved_advert_listing_records, $page_link, $page_no )
	...
	{
		$q = "SELECT * FROM title_dev_adverts LIMIT 0,20  ";
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r )
			return $r;
		else
