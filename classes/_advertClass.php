		...
		switch( $PhotoExtension )
		{
			...
			case "png":	case "PNG":
				...
				if( $memoryImage )
				{
					@imagecopyresampled($finalImage, $memoryImage, 0, 0, 0, 0, $newSize[0], $newSize[1], $size[0], $size[1]);
					@imageinterlace($finalImage, 0);
					@imagepng($finalImage, $image."_small.png", 100);
					@imagedestroy($memoryImage);
					return true;
				}	//	End of if( $memoryImage )
			break;
		...
		switch( $PhotoExtension )
		{
			...
			case "png":	case "PNG":
				...
				if( $memoryImage )
				{
					@imagecopyresampled($finalImage, $memoryImage, 0, 0, 0, 0, $newSize[0], $newSize[1], $size[0], $size[1]);
					@imageinterlace($finalImage, 0);
					@imagepng($finalImage, $image."_medium.png", 100);
					@imagedestroy($memoryImage);
					return true;
				}	//	End of if( $memoryImage )
			break;
...
function display_active_Customer_Advert_listing( $title_dev_customer_advert_listing_records, $page_link, $page_no )
	...
	{
		$q = "SELECT * FROM title_dev_advert_images WHERE isImage = 1 AND advertStatus = 1 ORDER BY addedDate ASC LIMIT 0,35  ";
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r )
			return $r;
		else
	...
	{
		$q = "SELECT * FROM title_dev_adverts WHERE isImage = 1 AND advertStatus = 1 ORDER BY advertId DESC LIMIT 0,35  ";
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r )
			return $r;
		else
	...
	{
		$q = "SELECT * FROM title_dev_adverts WHERE isImage = 1 AND advertStatus = 1 ORDER BY addedDate DESC LIMIT 0,6  ";
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r )
			return $r;
		else
...
function display_saved_Customer_Advert_listing( $title_dev_customer_saved_advert_listing_records, $page_link, $page_no )
	...
	{
		$q = "SELECT * FROM title_dev_adverts WHERE isImage = 1 AND advertStatus = 1 ORDER BY advertId DESC LIMIT 0,7  ";
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r )
			return $r;
		else
